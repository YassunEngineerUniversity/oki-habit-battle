  ...
  return (
    <div className="w-full max-w-[375px] fixed top-0 left-0 right-0 m-auto bg-white z-10 border-b border-gray-200">
      <div className="relative w-full">
        {backLink? (
          <Link href={backLink} className="absolute top-3 left-3 flex block cursor-pointer"> 
            <MdArrowBackIos width={16} height={16}  className="w-4 h-4 block"/>
          </Link>
        ): (
          <Link href="/profile" className="absolute top-1 left-3 flex block cursor-pointer"> 
            {profile ? (
        ...
        <h1 className="text-center font-bold text-[15px] pt-2 pb-2">{ title }</h1>
      </div>
    </div>
  )
}
