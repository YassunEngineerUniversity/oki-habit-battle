...
RSpec.describe Battles::BattleUpdateJob, type: :job do
  ...
  describe "perform" do
    let(:scheduled_jobs) { double('scheduled_jobs', display_class: 'Battles::BattleUpdateStatusJob') }
    ...
    context "バトルIDが無効でBattleが見つからない場合" do
      it "NotFoundParameterErrorが発生する" do
        expect {
          Battles::BattleUpdateJob.new.perform(nil, battle.battle_start_date.to_s)
    ...
    context "バトル開始時間が無効でBattleが見つからない場合" do
      it "NotFoundParameterErrorが発生する" do
        expect {
          Battles::BattleUpdateJob.new.perform(battle.id, nil)
        }.to raise_error(NotFoundParameterError)
      end
    end
    ...
    context "ScheduledJobが見つからない場合" do
      before do
        allow(scheduled_set).to receive(:scan).with("Battles::BattleUpdateStatusJob").and_return([])
      end
      it "NotFoundScheduledJobErrorが発生する" do
        expect {
          Battles::BattleUpdateJob.new.perform(battle.id, battle.battle_start_date.to_s)
